...
class DataRetrieval:
    ...
    def save_prediction_data(self, languages):
        ...
        data = pd.concat([self.get_light_features(), self.get_commit_density(), messages_features[0], messages_features[1]], axis=1)
        label = self.get_query_result(self.is_conflict_query.format())
        data_label = pd.merge(data, label, left_on='merge_commit', right_on='merge_commit', how='inner')
        data = data_label.drop(['merge_commit', 'merge_commit_temp', 'is_conflict', 'language_y'], axis=1).dropna()
        label = data_label[['language_y', 'is_conflict']].dropna()
        ...
        for language in languages:
            ...
            temp_data = data[data['language_x'] == language].drop('language_x', axis=1)
            temp_label = label[label['language_y'] == language].drop('language_y', axis=1)
    ...
    def get_commit_density(self):
        ...
        return pd.concat([density_paren1['merge_commit'], density_diff], axis=1).rename(columns={'merge_commit': 'merge_commit_temp'})
